...
//	To be used only with https://cleasbycode.co.uk/imgprmt/index/
...
void 
	...
	// For certain characters to display correctly within the html prompt, we need to find and replace them with
	// the correct html entity codes. For example, the word "café", the é (Latin small letter e with Acute character),
	// is replaced with html entity code "&#233"; Note, this is not an exhaustive list.
	Replace_Special_Chars(std::vector<wchar_t>& str),
...
void Open_Files(char* argv[]) {
	...
	std::string
		prompt,
		link,
		output_name = "imgprmt_pic.jpg";

	// Search the vector for certain characters and replace them with html entity codes
	Replace_Special_Chars(Prompt_Vec);

	// We have now replaced found wide characters and others with html entity codes, so no wide characters should exist in
	// the prompt. Convert the vector contents back to a standard string so that we can insert it
	// into the vector, ProfileVec. ProfileVec, in-turn, will be inserted into ImageVec and written out to file.
	std::transform(Prompt_Vec.begin(), Prompt_Vec.end(), std::back_inserter(prompt), [](wchar_t c) {
		return (char)c;
		});

	// Do the same for the url vector.
	std::transform(Url_Vec.begin(), Url_Vec.end(), std::back_inserter(link), [](wchar_t c) {
		return (char)c;
		});
	...
	Profile_Vec.insert(Profile_Vec.begin() + LINK_INSERT_INDEX, link.begin(), link.end());
...
void Replace_Special_Chars(std::vector<wchar_t>& str) {
	...
	// Current wide character values that are replaced with the html entity codes.
        ...
        constexpr wchar_t Tag[123]{
                11013, 11014, 11015, 11157, 10145, 9828, 9824, 8592, 8593, 8594, 8595, 8596, 8597, 8226, 8217, 8211, 8212,
                960, 351, 324, 322, 287, 263, 255, 254, 253, 252, 251, 250, 249, 248, 247, 246, 245, 244, 243, 242, 241,
                ...
                177, 176, 175, 174, 173, 172, 171, 170, 169, 168, 167, 166, 165, 164, 163, 162, 161, 144, 142, 39, 13, 10
        };

        // Check each character of the w_prompt string for wide characters and replace them with the html entity string.
        for (std::size_t i = str.size(); i != -1; i--) {
                wchar_t c = str[i];
                for (int x = 0; x != 123; x++) {
                        if (c == Tag[x]) {
