...
uint_fast32_t clear_icanon(void)
{
...
void Replace_Special_Chars(std::wstring& str) {
	...
	// the correct html entity code. For example, for the word "café", the é (Latin small letter e with acute),
	...
	wchar_t Tag[118]{
		11013, 11014, 11015, 11157, 10145, 8592, 8593, 8594, 8595, 8596, 8597, 8217, 8211, 8212, 960, 351, 324, 322,
		...
		169, 168, 167, 166, 165, 164, 163, 162, 161, 144, 142, 39
	};
...
void Open_Files(char* argv[]) {
	...
	std::wstring
		wide_url_link,
		wide_output_name = L"imgprmt_pic.jpg";
	...
	std::getline(std::wcin, wide_url_link);

	std::wcout << L"\nType or Paste Your Image Prompt / Description\n\n: ";

	const size_t WCIN_BUFFER_SIZE = 8000;  // The default string length for "wcin" seems to be just over 2000 characters. 
	// So we will increase it to 8000 to provide a more flexible size.  

	wchar_t wcin_buffer[WCIN_BUFFER_SIZE] = { 0 };

	std::wcin.rdbuf()->pubsetbuf(wcin_buffer, WCIN_BUFFER_SIZE); // Set the new "wcin" buffer size limt, as above.
	...
	std::transform(wide_url_link.begin(), wide_url_link.end(), std::back_inserter(link), [](wchar_t c) {
		return (char)c;
		});
	...
	std::wcout << L"\nCreated output file: \"" + wide_output_name + L" " << Image_Vec.size() << " " << "Bytes\"\n\n";
}
