#include "imgprmt_template.hpp"

#include <algorithm>
#include <array>
#include <cstddef>
#include <cstdint>
#include <cstring>
#include <format>
#include <iostream>
#include <limits>
#include <optional>
#include <print>
#include <ranges>
#include <span>
#include <stdexcept>
#include <string_view>
#include <utility>

namespace imgprmt::polyglot {
namespace {
constexpr std::size_t PROFILE_VEC_INTERNAL_DIFF = 38;
constexpr std::size_t PROFILE_VEC_MAIN_DIFF = 22;
constexpr std::size_t PROFILE_VEC_INSERT_INDEX = 0x14;
constexpr std::size_t PROFILE_VEC_SIZE_FIELD_INDEX = 0x28;
constexpr std::size_t SEGMENT_VEC_SIZE_FIELD_INDEX = 0x16;
constexpr std::size_t SEGMENT_VEC_INSERT_INDEX = 0xE3;
constexpr std::size_t SEGMENT_VEC_START_INDEX = 0x17;
constexpr std::size_t MAX_SEGMENT_SIZE = 65534;
constexpr std::size_t TWITTER_SEGMENT_LIMIT = 10 * 1024;
constexpr std::size_t EXIF_SIZE_FIELD_INDEX = 0x04;
constexpr std::size_t EXIF_ARTIST_SIZE_FIELD_INDEX = 0x4A;
constexpr std::size_t EXIF_SEGMENT_ARTIST_SIZE_DIFF = 0x8C;
constexpr std::size_t VALUE_BYTE_LENGTH = 4;

[[nodiscard]] std::size_t checked_add(std::size_t a, std::size_t b, std::string_view context) {
    if (b > std::numeric_limits<std::size_t>::max() - a) {
        throw std::runtime_error(std::format("{} size overflow.", context));
    }
    return a + b;
}

[[nodiscard]] std::optional<std::size_t> search_sig(
    std::span<const Byte> v,
    std::span<const Byte> sig,
    std::size_t limit = 0,
    std::size_t from = 0) {
    const std::size_t end = (limit == 0 || limit > v.size()) ? v.size() : limit;
    if (sig.empty() || from >= end || sig.size() > end - from) {
        return std::nullopt;
    }

    const Byte* const base = v.data();
    const std::size_t last_start = end - sig.size();
    std::size_t pos = from;
    while (pos <= last_start) {
        const void* match = std::memchr(base + pos, sig.front(), last_start - pos + 1);
        if (!match) {
            return std::nullopt;
        }

        pos = static_cast<const Byte*>(match) - base;
        if (sig.size() == 1 || std::memcmp(base + pos + 1, sig.data() + 1, sig.size() - 1) == 0) {
            return pos;
        }
        ++pos;
    }
    return std::nullopt;
}

void update_value(vBytes& vec, std::size_t index, std::size_t value, std::size_t length = 2) {
    if (length != 2 && length != 4) {
        throw std::invalid_argument("update_value: Unsupported field length.");
    }
    if (length > vec.size() || index > vec.size() - length) {
        throw std::out_of_range("update_value: Index out of bounds.");
    }

    const std::size_t max_value = (length == 2)
        ? static_cast<std::size_t>(std::numeric_limits<std::uint16_t>::max())
        : static_cast<std::size_t>(std::numeric_limits<std::uint32_t>::max());
    if (value > max_value) {
        throw std::out_of_range(std::format("update_value: Value overflow for {}-byte field.", length));
    }

    for (std::size_t i = 0; i < length; ++i) {
        const std::size_t shift = 8 * (length - i - 1);
        vec[index + i] = static_cast<Byte>((value >> shift) & 0xFFu);
    }
}

[[nodiscard]] std::optional<std::size_t> search_text(
    std::span<const Byte> haystack,
    std::string_view needle,
    std::size_t limit = 0,
    std::size_t from = 0) {
    if (needle.empty()) {
        return std::nullopt;
    }
    auto needle_bytes = std::span<const Byte>(reinterpret_cast<const Byte*>(needle.data()), needle.size());
    return search_sig(haystack, needle_bytes, limit, from);
}

[[nodiscard]] std::optional<std::size_t> find_tag_end(std::span<const Byte> data, std::size_t from) {
    if (from >= data.size()) {
        return std::nullopt;
    }

    for (std::size_t i = from; i < data.size(); ++i) {
        if (data[i] == static_cast<Byte>('>')) {
            return i;
        }
    }
    return std::nullopt;
}

void strip_external_link_tags(vBytes& vec) {
    constexpr auto LINK_PREFIX = std::to_array<Byte>({'<', 'l', 'i', 'n', 'k'});
    constexpr std::array<std::string_view, 2> EXTERNAL_DOMAINS{
        "fonts.googleapis.com",
        "fonts.gstatic.com",
    };

    std::vector<std::pair<std::size_t, std::size_t>> erase_ranges;
    for (std::size_t from = 0;;) {
        auto tag_start_opt = search_sig(vec, LINK_PREFIX, 0, from);
        if (!tag_start_opt) {
            break;
        }

        const std::size_t tag_start = *tag_start_opt;
        auto tag_end_opt = find_tag_end(vec, tag_start + LINK_PREFIX.size());
        if (!tag_end_opt) {
            throw std::runtime_error("Template Error: Failed to parse external link tag.");
        }

        const std::size_t tag_end = *tag_end_opt;
        const auto tag = std::span<const Byte>(vec.data() + tag_start, tag_end - tag_start + 1);
        const bool is_external_link = std::ranges::any_of(EXTERNAL_DOMAINS, [&](std::string_view domain) {
            return search_text(tag, domain).has_value();
        });
        if (is_external_link) {
            erase_ranges.emplace_back(tag_start, tag_end + 1);
        }

        from = tag_end + 1;
    }

    for (auto it = erase_ranges.rbegin(); it != erase_ranges.rend(); ++it) {
        vec.erase(vec.begin() + static_cast<std::ptrdiff_t>(it->first),
                  vec.begin() + static_cast<std::ptrdiff_t>(it->second));
    }
}

struct TemplateReplacement {
    std::size_t pos = 0;
    std::span<const Byte> marker;
    std::string_view replacement;
};

[[nodiscard]] TemplateReplacement find_replacement(
    const vBytes& vec,
    std::span<const Byte> marker,
    std::string_view replacement,
    std::string_view marker_name) {
    auto pos_opt = search_sig(vec, marker);
    if (!pos_opt) {
        throw std::runtime_error(std::format("Template Error: Missing {} marker.", marker_name));
    }
    return TemplateReplacement{
        .pos = *pos_opt,
        .marker = marker,
        .replacement = replacement,
    };
}

void replace_template_markers(
    vBytes& vec,
    std::span<const Byte> url_marker,
    std::string_view url_replacement,
    std::span<const Byte> prompt_marker,
    std::string_view prompt_replacement) {
    std::array replacements{
        find_replacement(vec, url_marker, url_replacement, "URL"),
        find_replacement(vec, prompt_marker, prompt_replacement, "PROMPT"),
    };

    std::ranges::sort(replacements, {}, &TemplateReplacement::pos);
    if (replacements[0].pos + replacements[0].marker.size() > replacements[1].pos) {
        throw std::runtime_error("Template Error: Replacement markers overlap.");
    }

    std::size_t output_size = vec.size();
    for (const auto& replacement : replacements) {
        output_size -= replacement.marker.size();
        output_size = checked_add(output_size, replacement.replacement.size(), "Template");
    }

    vBytes out;
    out.reserve(output_size);

    std::size_t cursor = 0;
    for (const auto& replacement : replacements) {
        out.insert(out.end(), vec.begin() + static_cast<std::ptrdiff_t>(cursor),
                   vec.begin() + static_cast<std::ptrdiff_t>(replacement.pos));
        out.insert(out.end(), replacement.replacement.begin(), replacement.replacement.end());
        cursor = replacement.pos + replacement.marker.size();
    }
    out.insert(out.end(), vec.begin() + static_cast<std::ptrdiff_t>(cursor), vec.end());
    vec.swap(out);
}

void apply_segment_mode(SegmentMode mode, vBytes& segment_vec, vBytes& profile_vec, vBytes& bluesky_vec) {
    if (mode == SegmentMode::Default) {
        segment_vec.insert(segment_vec.begin() + PROFILE_VEC_INSERT_INDEX, profile_vec.begin(), profile_vec.end());
    } else {
        bluesky_vec.insert(bluesky_vec.begin() + SEGMENT_VEC_INSERT_INDEX, segment_vec.begin() + SEGMENT_VEC_START_INDEX, segment_vec.end());
        segment_vec.swap(bluesky_vec);
    }
}

void replace_content_markers(vBytes& segment_vec, std::string_view utf8_prompt, std::string_view utf8_url) {
    constexpr auto PROMPT_INSERT_MARKER = std::to_array<Byte>({'%','%','P','R','O','M','P','T','%','%'});
    constexpr auto URL_INSERT_MARKER = std::to_array<Byte>({'%','%','U','R','L','%','%'});

    const std::size_t max_pre_replacement_size =
        checked_add(MAX_SEGMENT_SIZE, checked_add(PROMPT_INSERT_MARKER.size(), URL_INSERT_MARKER.size(), "Template"), "Template");
    std::size_t reserve_size = checked_add(segment_vec.size(), utf8_prompt.size(), "Template");
    reserve_size = checked_add(reserve_size, utf8_url.size(), "Template");
    if (reserve_size > max_pre_replacement_size) {
        throw std::runtime_error("File Size Error: Data content size exceeds the maximum segment limit.");
    }

    replace_template_markers(segment_vec, URL_INSERT_MARKER, utf8_url, PROMPT_INSERT_MARKER, utf8_prompt);
}

void warn_if_twitter_incompatible(SegmentMode mode, std::size_t segment_size) {
    if (mode == SegmentMode::Default && segment_size > TWITTER_SEGMENT_LIMIT) {
        std::println(std::cerr, "\n\nWarning: Data content exceeds the maximum segment size limit for X-Twitter.\n\t Image will not be compatible for posting on that platform.");
    }
}

void update_bluesky_size_fields(vBytes& segment_vec, std::size_t segment_size) {
    if (segment_size < 4) {
        throw std::runtime_error("File Size Error: Invalid Bluesky segment size.");
    }
    segment_size -= 4;

    if (segment_size < EXIF_SEGMENT_ARTIST_SIZE_DIFF) {
        throw std::runtime_error("File Size Error: Invalid EXIF artist segment size.");
    }
    const std::size_t EXIF_ARTIST_SIZE = segment_size - EXIF_SEGMENT_ARTIST_SIZE_DIFF;

    update_value(segment_vec, EXIF_SIZE_FIELD_INDEX, segment_size);
    update_value(segment_vec, EXIF_ARTIST_SIZE_FIELD_INDEX, EXIF_ARTIST_SIZE, VALUE_BYTE_LENGTH);
}

void update_default_size_fields(vBytes& segment_vec, std::size_t segment_size) {
    if (segment_size < PROFILE_VEC_MAIN_DIFF || segment_size < PROFILE_VEC_INTERNAL_DIFF) {
        throw std::runtime_error("File Size Error: Invalid profile segment size.");
    }
    update_value(segment_vec, SEGMENT_VEC_SIZE_FIELD_INDEX, (segment_size - PROFILE_VEC_MAIN_DIFF));
    update_value(segment_vec, PROFILE_VEC_SIZE_FIELD_INDEX, (segment_size - PROFILE_VEC_INTERNAL_DIFF));
}

void update_segment_size_fields(SegmentMode mode, vBytes& segment_vec) {
    const std::size_t segment_size = segment_vec.size();
    warn_if_twitter_incompatible(mode, segment_size);

    if (segment_size > MAX_SEGMENT_SIZE) {
        throw std::runtime_error("File Size Error: Data content size exceeds the maximum segment limit.");
    }

    if (mode == SegmentMode::Bluesky) {
        update_bluesky_size_fields(segment_vec, segment_size);
    } else {
        update_default_size_fields(segment_vec, segment_size);
    }
}
constexpr auto PROFILE_DATA = std::to_array<Byte>({
    
});

constexpr auto BLUESKY_DATA = std::to_array<Byte>({
    
});

constexpr auto SEGMENT_DATA = std::to_array<Byte>({
    
});
}  // namespace

vBytes build_segment(SegmentMode mode, std::string utf8_prompt, std::string utf8_url) {
	vBytes profile_vec(PROFILE_DATA.begin(), PROFILE_DATA.end());
	vBytes bluesky_vec(BLUESKY_DATA.begin(), BLUESKY_DATA.end());
	vBytes segment_vec(SEGMENT_DATA.begin(), SEGMENT_DATA.end());
	strip_external_link_tags(segment_vec);

	apply_segment_mode(mode, segment_vec, profile_vec, bluesky_vec);
	replace_content_markers(segment_vec, utf8_prompt, utf8_url);
	update_segment_size_fields(mode, segment_vec);

	return segment_vec;
}
}  // namespace imgprmt::polyglot
