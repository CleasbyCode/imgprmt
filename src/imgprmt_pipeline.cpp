...
namespace {
[[nodiscard]] std::optional<std::size_t> find_jpeg_marker(std::span<const Byte> jpg, Byte marker) {
	...
	std::size_t pos = 2;
	while (pos < jpg.size()) {
		const void* marker_start = std::memchr(jpg.data() + pos, 0xFF, jpg.size() - pos);
		if (!marker_start) {
			return std::nullopt;
		}
		pos = static_cast<const Byte*>(marker_start) - jpg.data();
...
prepared_cover prepare_cover_image(const std::filesystem::path& image_file_path, bool is_bluesky_mode) {
	...
	auto dqt_pos_opt = find_jpeg_marker(jpg_vec, DQT_MARKER);
	if (!dqt_pos_opt) {
		throw std::runtime_error("Image File Error: No DQT segment found (corrupt or unsupported JPG).");
	}

	const std::size_t dqt_pos = *dqt_pos_opt;
