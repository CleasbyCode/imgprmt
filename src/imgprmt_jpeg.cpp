...
std::optional<std::uint16_t> exifOrientation(std::span<const Byte> jpg) {
	...
	if (segment_length > jpg.size() - pos - 2) {
		return std::nullopt;
	}
	...
	auto read16 = [&](std::size_t offset) -> std::uint16_t {
		if (offset > tiff_data.size() || tiff_data.size() - offset < 2) {
			return 0;
		}
		std::uint16_t value;
	...
	auto read32 = [&](std::size_t offset) -> std::uint32_t {
		if (offset > tiff_data.size() || tiff_data.size() - offset < 4) {
			return 0;
		}
		std::uint32_t value;
...
int estimateImageQuality(std::span<const Byte> jpg) {
	...
	if (length < 2 || length > jpg.size() - pos - 2) {
		return 80;
	}
