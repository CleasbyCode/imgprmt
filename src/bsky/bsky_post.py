...
def _normalize_pds_url(pds_url: str) -> str:
    ...
    if parsed.scheme != "https":
        ...
        if not local_http:
            raise ValueError("PDS URL must use HTTPS, except for localhost development.")
    if not parsed.hostname or parsed.username or parsed.password:
        raise ValueError("PDS URL must include a host and must not include credentials.")
    if parsed.path not in ("", "/") or parsed.params or parsed.query or parsed.fragment:
        raise ValueError("PDS URL must be an origin only, without path, query, or fragment.")
    return pds_url.rstrip("/")
    ...
    for i, ip in enumerate(image_paths):
        ...
        with Image.open(io.BytesIO(img_bytes)) as img:
            ...
            if width <= 0 or height <= 0 or width * height > MAX_IMAGE_PIXELS:
                raise ValueError(
                    f"Image dimensions exceed safe limit of {MAX_IMAGE_PIXELS:,} pixels."
                )
            img.verify()
...
def main():
    ...
    try:
        args.pds_url = _normalize_pds_url(args.pds_url)
    ...
    embed_modes = sum(1 for enabled in (bool(args.image), bool(args.embed_url), bool(args.embed_ref)) if enabled)
    if embed_modes > 1:
        exit_error("Error: Choose only one embed source: --image, --embed-url, or --embed-ref.")
    if args.alt_text and not args.image:
        exit_error("Error: --alt-text requires at least one --image.")
    if args.alt_text and len(args.alt_text) > len(args.image):
        exit_error("Error: More --alt-text values were provided than --image values.")
